...
std::vector<AboutPanel::Section> AboutPanel::makeSections (bool english)
{
    ...
    if (english)
    {
        ...
        s.push_back ({ utf8 ("Overview"), utf8 (
            ...
            "Voice sum → Fibonacci gate → FX drive (dist/sat/fold) → Golden Delay →\n"
            "FX reverb → master gain (ramped) → output.\n"
            "\n"
            "MODULATION\n"
    ...
    s.push_back ({ utf8 ("Przegląd"), utf8 (
        ...
        "Suma głosów → gate Fibonacciego → FX drive (dist/sat/fold) → Golden Delay →\n"
        "pogłos FX → master gain (z rampą) → wyjście.\n"
        "\n"
        "MODULACJA\n"
